...
# @Last Modified time: 2019-09-27 21:27:55
...
def component():
	if isinstance(current_user._get_current_object(), Admin):
		...
		next_url = url_for(
			'component', page=items.next_num) if items.has_next else None
		prev_url = url_for(
			'component', page=items.prev_num) if items.has_prev else None
		...
		if add_form.validate_on_submit():
			...
			if Component.query.filter_by(name=add_form.name.data, type=add_form.type.data, department=department).first() is not None:
				flash("已存在这个元器件")
			...
			flash("元器件添加成功")
			return redirect(url_for('component'))
		return render_template('admin/component.html', items=items.items, form=add_form,
							   next_url=next_url,
							   prev_url=prev_url
							   )
...
def info():
	...
	if isinstance(current_user._get_current_object(), Admin):
		page = request.args.get('page',1,type=int)
		data = db.session.query(Application, Student, Component).join(
			Student).join(Component).order_by(Application.time.desc()).paginate(page,app.config['DATA_PER_PAGE'],False)
		next_url = url_for(
			'info', page=data.next_num) if data.has_next else None
		prev_url = url_for(
			'info', page=data.prev_num) if data.has_prev else None
		items = [dict(zip(result.keys(), result)) for result in data.items]
		return render_template('admin/info.html', items=items,next_url=next_url,prev_url=prev_url)
	elif isinstance(current_user._get_current_object(), Student):
		page = request.args.get('page',1,type=int)

		data = db.session.query(Application, Student, Component).join(Student).join(
			Component).filter(Application.student_id == current_user.student_num).order_by(Application.id.desc()).paginate(page,app.config['DATA_PER_PAGE'],False)
		next_url = url_for(
			'info', page=data.next_num) if data.has_next else None
		prev_url = url_for(
			'info', page=data.prev_num) if data.has_prev else None
		items = [dict(zip(result.keys(), result)) for result in data.items]
		return render_template('student/info.html', items=items,next_url=next_url,prev_url=prev_url)
...
def modify(component_id):
	...
	if request.method == 'POST' or request.method == 'GET':
		...
		totalPrice = round(float(unitPrice)*float(quantity), 2)
		...
		if totalPrice != component.totalPrice:
